                    ...
                    <div class="ticket-reply-area">
                        @forelse($ticket_reply as $ticket)
                            <div class="single-reply {{ $ticket->admin_id != null ? 'admin-reply' : '' }} mt-5">
                                <span class="fs-sm sp_text_secondary mb-2">{{ 'Reply In' }}
                                ...
                                @if ($ticket->file)
                                    <p class="mb-0 mt-2">
                                        <a class="sp_site_color" href="{{ route('user.ticket.download', $ticket->id) }}"> <i class="fas fa-cloud-download-alt"></i> {{ __('View Attachement') }}</a>
                                    </p>
                                @endif
                            </div>
                            <hr>
