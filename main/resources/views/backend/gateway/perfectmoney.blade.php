                    ...
                    <form action="{{ route('admin.payment.update.online', $gateway->id) }}" method="post"
                        ...
                        <div class="row">
                            <div class="form-group col-md-3 mb-3">
                                <label class="col-form-label">{{ __('Perfect Money Image') }}</label>
                            ...
                            <div class="col-md-9">
                                <div class="row">
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <input type="text" name="parameter[accountid]" class="form-control"
                                            value="{{ env('DEMO') ? '------' : $gateway->parameter->accountid ?? old('accountid') }}">
                                    </div>


                                    <div class="form-group col-md-6">
                                        <label for="">{{ __('Passphrase') }}</label>
                                        <input type="text" name="parameter[passphrase]" class="form-control"
                                            value="{{ env('DEMO') ? '------' : $gateway->parameter->passphrase ?? old('passphrase') }}">
                                    </div>
                            ...
                            <div class="col-md-12">
                                <button type="submit"
                                    class="btn btn-primary float-right">{{ __('Update PerfectMoney Information') }}</button>
                            </div>
                        </div>
                    </form>
                </div>
