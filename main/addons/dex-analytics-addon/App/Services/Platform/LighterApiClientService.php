...
class LighterApiClientService
{
    ...
    protected function get(string $endpointKey, array $params = []): array
    {
        ...
        if (!$endpoint) {
            return [
                'success' => false,
                'message' => 'Missing Lighter endpoint configuration: ' . $endpointKey,
            ];
        }
        ...
        try {
            ...
            if (!$response->successful()) {
                return [
                    'success' => false,
                    'message' => 'Lighter request failed: ' . $response->status(),
                ];
            }
    ...
    {
        return config('dex-analytics.platforms.lighter', []);
    }
}
