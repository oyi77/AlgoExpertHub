...
class PerfectMoneyService
{
    ...
    public function success(Request $request)
    {
        if (session('type') === 'deposit') {
            $depost = Deposit::where('trx', $request->PAYMENT_ID)->first();
        } else {
            $depost = Payment::where('trx', $request->PAYMENT_ID)->first();
        }

        $gateway = $depost->gateway->parameter;

        $passphrase = strtoupper(md5($gateway->passphrase));

        define('ALTERNATE_PHRASE_HASH', $passphrase);
        define('PATH_TO_LOG', public_path());
        $string =
        ...
        if ($request->PAYEE_ACCOUNT == $gateway->accountid && $unit == $gateway->gateway_currency && $amount == $depost->total) {
            Helper::paymentSuccess($depost, 0, $transaction);
