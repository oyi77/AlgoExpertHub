        ...
        <div class="col-12">
            <div class="sp_site_card">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <h4 class="mb-0">{{ $title }}: {{ $source->name }}</h4>
                </div>
                ...
                <div class="alert alert-info mb-4">
                    ...
                    {{ __('Select a channel or group from the list below to forward messages from.') }}
                </div>
                ...
                @else
                    <form action="{{ route('user.channel-forwarding.select-channel.post', $source->id) }}" method="post">
                        @csrf
                        ...
                        @if (count(array_filter($dialogs, function($d) {
                            ...
                            <div class="mt-4">
                                <button type="submit" class="btn sp_theme_btn" id="submitBtn" disabled>
                                    <i class="las la-check"></i> {{ __('Select Channel') }}
                                </button>
                                <a href="{{ route('user.channel-forwarding.index') }}" class="btn btn-secondary">
                                    {{ __('Cancel') }}
                                </a>
                            </div>
                        @endif
