...
class ManualGatewayService
{
    public function updateOnlineGateway($request, $gateway)
    {
        if ($request->hasFile('image')) {
            $filename = Helper::saveImage($request->image, Helper::filePath('gateways', true), '', $gateway->image);
        }
    ...
    public function createMethod($request)
    {
        ...
        if ($request->hasFile('image')) {
            $filename = Helper::saveImage($request->image, Helper::filePath('gateways', true));
        }

        if ($request->hasFile('qr_code')) {
            $gatewayParameters['qr_code'] = Helper::saveImage($request->qr_code, Helper::filePath('gateways', true));
        }
    ...
    public function updateMethod($request, $gateway)
    {
        ...
        if ($request->hasFile('qr_code')) {
            $gatewayParameters['qr_code'] = Helper::saveImage($request->qr_code, Helper::filePath('gateways', true), '', $gateway->parameter->qr_code);
        }
