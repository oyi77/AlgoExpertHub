    ...
    public function handle()
    {
        ...
        try {
            ...
            if (!\Illuminate\Support\Facades\Schema::hasTable($tableName)) {
                $this->warn("Table '{$tableName}' does not exist. Skipping web scrape processing.");
                Log::info("ProcessWebScrapeChannels: Table '{$tableName}' does not exist, skipping.");
                return 0;
            }
            ...
            if (str_contains($e->getMessage(), "doesn't exist") || str_contains($e->getMessage(), 'Base table or view not found')) {
                $tableName = (new ChannelSource())->getTable();
                $this->warn("Table '{$tableName}' does not exist. Skipping web scrape processing.");
                Log::info("ProcessWebScrapeChannels: Database error - table does not exist, skipping.", ['error' => $e->getMessage()]);
                return 0;
            }
            throw $e;
        }
