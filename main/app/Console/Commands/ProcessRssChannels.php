...
class ProcessRssChannels extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'channel:process-rss';
     ...
     * @var string
     */
    protected $description = 'Process RSS feed channels and fetch new items';
    ...
    {
        $this->info('Processing RSS feed channels...');

        $channels = ChannelSource::where('type', 'rss')
            ->where('status', 'active')
            ->get();

        if ($channels->isEmpty()) {
            $this->info('No active RSS feed channels found.');
            return 0;
        }
        ...
        foreach ($channels as $channel) {
            try {
                $adapter = new RssAdapter($channel);
                ...
                $this->info("Processed channel: {$channel->name} ({$messages->count()} new items)");

            } catch (\Exception $e) {
                $this->error("Error processing channel {$channel->name}: " . $e->getMessage());
                Log::error("RSS processing error for channel {$channel->id}: " . $e->getMessage());
        ...
        $this->info("Processing complete. Processed: {$processed} items, Errors: {$errors}");
